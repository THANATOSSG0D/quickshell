...
C.CfgScroll {
  ...
  C.CfgToggle {
    label: "Mostrar saída";     checked: root.localShowSink
    colorAccent: root.colorAccent; colorTextDim: root.colorTextDim
    onToggled: root.changed({ volShowSink: !root.localShowSink })
  }
  C.CfgToggle {
    label: "Mostrar microfone"; checked: root.localShowSource
    colorAccent: root.colorAccent; colorTextDim: root.colorTextDim
    onToggled: root.changed({ volShowSource: !root.localShowSource })
  }
  ...
  Rectangle {
    ...
    RowLayout {
      ...
      Rectangle {
        ...
        Text { id: bypassLbl; anchors.centerIn: parent
  ...
  PresetList {
    width: parent.width; profiles: root.eeOutputProfiles; activeProfile: root.eeActiveOutput
    colorAccent: root.colorAccent; colorTextDim: root.colorTextDim; colorText: root.colorText
    onSelected: (name) => root.applyPreset("output", name)
  }

  C.CfgSection { title: "PRESET ENTRADA"; colorTextDim: root.colorTextDim }
  PresetList {
    width: parent.width; profiles: root.eeInputProfiles; activeProfile: root.eeActiveInput
    colorAccent: root.colorAccent; colorTextDim: root.colorTextDim; colorText: root.colorText
  ...
  C.CfgPalette {
    label: "Mudo"; value: root.pkVolMuted; colors: root.colors; overlay: root.overlay
    colorAccent: root.colorAccent; colorTextDim: root.colorTextDim
    colorText: root.colorText; colorSidebar: root.colorSidebar; colorDivider: root.colorDivider
    onEdited: (v) => root.changed({ pkVolMuted: v })
  }

  // ══════════════════════════════════════════════════════════════════════
  // Componente: painel de dispositivos (sink ou source)
  // Mostra o dispositivo padrão com slider de volume + seção expansível
  // com todos os outros dispositivos e seus sliders.
  // ══════════════════════════════════════════════════════════════════════
  component PwDevicePanel: Column {
    ...
    // Lista computada de dispositivos do tipo certo
    ...
    Rectangle {
      ...
      Column {
        ...
        // Cabeçalho: nome + badge "padrão"
        Row {
          width: parent.width
          spacing: 8

          Column {
            width: parent.width - padraoBadge.width - 8
            spacing: 2
            Text {
              width: parent.width
        ...
        // Slider de volume do dispositivo padrão
        VolumeRow {
          width:    parent.width
    ...
    Column {
      ...
      Repeater {
        ...
        delegate: Rectangle {
          ...
          width:  parent.width
          height: otherCol.implicitHeight + 14
          radius: 7
          ...
          Column {
            ...
            Row {
              ...
              Column {
                width: parent.width - setDefaultBtn.width - 8
                spacing: 2
                Text {
                  width: parent.width
  ...
  // ══════════════════════════════════════════════════════════════════════
  // Componente: linha de volume (ícone mute + track 0–150% + valor)
  // ══════════════════════════════════════════════════════════════════════
  component VolumeRow: Row {
    ...
    // Track
    Item {
      width:  parent.width - 22 - 40 - 16  // fill
      height: 20
